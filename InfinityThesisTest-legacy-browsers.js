...
async function experimentInit() {
  ...
  EndInstr = new core.Keyboard({psychoJS: psychoJS, clock: new util.Clock(), waitForStart: true});
...
function InstructionsRoutineBegin(snapshot) {
  return async function () {
    ...
    EndInstr.keys = undefined;
    EndInstr.rt = undefined;
    _EndInstr_allKeys = [];
    // keep track of which components have finished
    InstructionsComponents = [];
    InstructionsComponents.push(Instr);
    InstructionsComponents.push(EndInstr);
...
function InstructionsRoutineEachFrame() {
  return async function () {
    ...
    if (Instr.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      Instr.setAutoDraw(false);
    }
    
    // *EndInstr* updates
    if (t >= 0.0 && EndInstr.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      EndInstr.tStart = t;  // (not accounting for frame time here)
      EndInstr.frameNStart = frameN;  // exact frame index
      
      // keyboard checking is just starting
      psychoJS.window.callOnFlip(function() { EndInstr.clock.reset(); });  // t=0 on next screen flip
      psychoJS.window.callOnFlip(function() { EndInstr.start(); }); // start on screen flip
      psychoJS.window.callOnFlip(function() { EndInstr.clearEvents(); });
    }

    if (EndInstr.status === PsychoJS.Status.STARTED) {
      let theseKeys = EndInstr.getKeys({keyList: ['e'], waitRelease: false});
      _EndInstr_allKeys = _EndInstr_allKeys.concat(theseKeys);
      if (_EndInstr_allKeys.length > 0) {
        EndInstr.keys = _EndInstr_allKeys[_EndInstr_allKeys.length - 1].name;  // just the last key pressed
        EndInstr.rt = _EndInstr_allKeys[_EndInstr_allKeys.length - 1].rt;
...
function InstructionsRoutineEnd() {
  return async function () {
    ...
    psychoJS.experiment.addData('EndInstr.keys', EndInstr.keys);
    if (typeof EndInstr.keys !== 'undefined') {  // we had a response
        psychoJS.experiment.addData('EndInstr.rt', EndInstr.rt);
        routineTimer.reset();
        }
